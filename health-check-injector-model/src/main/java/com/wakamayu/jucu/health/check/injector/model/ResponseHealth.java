...
public class ResponseHealth implements Serializable {
	...
	private List<ResponseHealtCheck> checks;

	public ResponseHealth() {
		this.status = TypeStatus.UP;
		this.checks = new ArrayList<ResponseHealtCheck>();
	}

	@JsonIgnore
	public void addCheck(HealthCheckResponse checkResponse) {
		this.checks.add(new ResponseHealtCheck(checkResponse));
	}
	...
	public List<ResponseHealtCheck> getChecks() {
		return checks;
	}

	public void setChecks(List<ResponseHealtCheck> checks) {
		this.checks = checks;
	}
